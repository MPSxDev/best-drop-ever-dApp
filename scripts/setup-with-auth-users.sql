...
-- 1. Create all tables (same as setup-database.sql)
...
-- 4. Create RLS policies (same as setup-database.sql)
-- Profiles policies
...
-- 6. Create auth users and profiles
-- Note: This requires the service role key to create auth users
-- You'll need to run this part using the Supabase service role

-- Create auth users first (this needs to be done via API or Supabase dashboard)
-- For now, we'll create profiles without user_id and you can link them later

-- Insert test DJ profiles (without user_id for now)
...
-- Insert test FAN profiles (without user_id for now)
INSERT INTO public.profiles (
  user_id,
  handle,
  display_name,
...
-- Success message
SELECT 'Database setup with auth users completed successfully!' as status;
